...
const SuperAdminDashboard = () => {
  ...
  // Order stats
  const [orderStats, setOrderStats] = useState({ revenue: 0, total: 0, active: 0, delivered: 0, cancelled: 0, avgOrder: 0 });
  ...
  const fetchOrderStats = async () => {
    ...
    const [ordersRes, recentRes] = await Promise.all([
      supabase.from('orders').select('total, status, created_at').gte('created_at', from).lte('created_at', to),
  ...
  const fetchTopSellers = async () => {
    const { data: orderItems } = await supabase.from('order_items').select('menu_item_id, name, quantity, price');
      ...
      <div className="px-4 py-4 space-y-5">
            ...
            >
              <div className="grid grid-cols-3 gap-1">
                {DATE_OPTIONS.filter(d => d.key !== 'custom').map(d => (
                  <button
                    key={d.key}
                    onClick={() => { setDateRange(d.key); setShowDatePicker(false); }}
                    className={`px-3 py-2 rounded-lg text-xs font-medium transition-colors ${
                      dateRange === d.key ? 'bg-secondary/20 text-secondary' : 'text-muted-foreground hover:bg-muted'
                    }`}
                  >
                    {d.label}
                  </button>
              ...
              <div className="border-t border-border mt-2 pt-2">
                ...
                <button
                  ...
                  className="w-full mt-2 py-1.5 bg-secondary/20 text-secondary rounded-lg text-xs font-medium"
                >
                  Apply Custom
                </button>
              </div>
        ...
        <div>
          ...
          {orderStats.avgOrder > 0 && (
            <div className="mt-2 bg-card border border-border rounded-xl p-3 flex items-center justify-between">
        ...
        <div>
          ...
          <motion.div
            ...
            className="bg-gradient-to-r from-secondary/10 to-primary/10 border border-secondary/20 rounded-xl p-4 flex items-center gap-4"
          >
              ...
              {topSellers.map((item, idx) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-muted-foreground">{item.qty} orders</p>
                  </div>
          ...
          {recentOrders.length === 0 ? (
            ...
            <div className="space-y-2">
              {recentOrders.map(order => (
                <div key={order.id} className="bg-card border border-border rounded-xl p-3">
